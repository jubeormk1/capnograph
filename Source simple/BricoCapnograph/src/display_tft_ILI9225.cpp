...
#define MILLIS_DISPLAY_GRAPH 10000 // milliseconds to display in the graph
  ...
  void setupGraph(void)
  {
    ...
    // FixMe: Not of display business. Configure in main() or a sensor header
    sensorSampleTime = 100; // I forgot why I am setting the sensor Sample time here lol

    while (totalMsDisplay < MILLIS_DISPLAY_GRAPH) // We want to display 10 seconds
    {
      totalMsDisplay = sensorSampleTime * ++samplesPerPixel * display_y_size;
    }
    // FixMe: setting sensor parameters from the display without interface
